...
class AsyncRolloutRequest(BaseModel):
    ...
    def truncate_output_ids(
        ...
        # Sync multi_modal_inputs with the (possibly truncated) input_ids.
        # Truncating input_ids may have removed image tokens; the corresponding pixel_values
        # and image_grid_thw must be trimmed accordingly, otherwise compute_log_prob will
        # raise "Image features and image tokens do not match".
        if self.multi_modal_inputs and "pixel_values" in self.multi_modal_inputs:
            self._sync_multi_modal_inputs_after_truncation(processing_class)

    def _sync_multi_modal_inputs_after_truncation(
        self, processing_class: PreTrainedTokenizer | PreTrainedTokenizerFast | ProcessorMixin
    ) -> None:
        """Trim pixel_values / image_grid_thw so they match the image tokens left in input_ids."""
        image_grid_thw = self.multi_modal_inputs.get("image_grid_thw")
        ...
        # How many image-token slots remain in the (truncated) input_ids?
        n_remaining_tokens = int((self.input_ids == image_token_id).sum())

        # Per-image: number of image tokens and raw patches
        per_image_tokens = []
        per_image_patches = []
        for thw in image_grid_thw:
            t, h, w = int(thw[0]), int(thw[1]), int(thw[2])
            per_image_tokens.append(t * h * w // (merge_size * merge_size))
            per_image_patches.append(t * h * w)

        # Greedily keep images from the start while their tokens fit
        n_keep = 0
        token_sum = 0
        for n_tok in per_image_tokens:
            if token_sum + n_tok <= n_remaining_tokens:
                token_sum += n_tok
                n_keep += 1
            else:
                break

        if n_keep == len(per_image_tokens):
            return  # Nothing was cut

        # Trim pixel_values and image_grid_thw to the kept images
        n_patches_keep = sum(per_image_patches[:n_keep])
        self.multi_modal_inputs["pixel_values"] = self.multi_modal_inputs["pixel_values"][:n_patches_keep]
        self.multi_modal_inputs["image_grid_thw"] = image_grid_thw[:n_keep]
