...
def _to_json_serializable(value):
    ...
    if torch.is_tensor(value):
        return _to_json_serializable(value.detach().cpu().tolist())
    if isinstance(value, dict):
        return {str(_to_json_serializable(k)): _to_json_serializable(v) for k, v in value.items()}
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        ...
        for i in range(n):
            ...
            lines.append(json.dumps(_to_json_serializable(entry), ensure_ascii=False, default=str))
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Enable full trajectory trace saving during validation if configured
            _val_trace_dir = self.config.trainer.get("validation_trace_dir", None)
            if _val_trace_dir:
                test_gen_batch.meta_info["save_trace"] = True
                test_gen_batch.meta_info["trace_save_dir"] = os.path.join(
                    _val_trace_dir, f"val_step_{self.global_steps}"
                )
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Determine if this step should save complete trajectory traces (aligned with checkpoint saving)
                _save_freq = self.config.trainer.save_freq
                _is_save_step = _save_freq > 0 and (
                    self.global_steps % _save_freq == 0 or self.global_steps >= self.total_training_steps
                )
                gen_batch.meta_info["save_trace"] = _is_save_step
                if _is_save_step:
                    gen_batch.meta_info["trace_save_dir"] = os.path.join(
