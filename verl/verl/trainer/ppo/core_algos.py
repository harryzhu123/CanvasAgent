...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        # --- Diagnostic: log per-group reward stats to detect zero-variance collapse ---
        all_stds = [id2std[idx].item() for idx in id2score]
        all_means = [id2mean[idx].item() for idx in id2score]
        raw_scores_list = [s.item() for group in id2score.values() for s in group]
        n_groups = len(id2score)
        n_zero_std = sum(1 for s in all_stds if s < 1e-4)
        print(
            f"[GRPO_ADV] groups={n_groups} group_size={bsz // max(n_groups, 1)} "
            f"| reward: mean={sum(raw_scores_list)/len(raw_scores_list):.4f} "
            f"min={min(raw_scores_list):.4f} max={max(raw_scores_list):.4f} "
            f"| group_std: mean={sum(all_stds)/len(all_stds):.4f} "
            f"min={min(all_stds):.4f} max={max(all_stds):.4f} "
            f"zero_var_groups={n_zero_std}/{n_groups}",
            flush=True,
        )
