...
_NON_TENSOR_REWARD_METRIC_SKIP_KEYS = {
    "__num_turns__",
    "tool_call_counts",
    "tool_call_count",
    "uid",
...
def _extract_numeric_array(values: Any) -> np.ndarray | None:
    """Best-effort conversion of non-tensor batch values into a numeric 1D array."""
...
def _compute_non_tensor_reward_metrics(batch: DataProto) -> dict[str, Any]:
    """Aggregate numeric reward extra fields from non_tensor_batch for logging."""
    metrics = {}
    for key, values in batch.non_tensor_batch.items():
        if key in _NON_TENSOR_REWARD_METRIC_SKIP_KEYS:
            continue
        numeric_values = _extract_numeric_array(values)
        if numeric_values is None or numeric_values.size == 0:
            continue
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    score_std = torch.std(non_aborted_sequence_score).detach().item() if non_aborted_sequence_score.numel() > 1 else 0.0
    ...
    elif "tool_call_count" in batch.non_tensor_batch:
        tool_call_counts = _extract_numeric_array(batch.non_tensor_batch["tool_call_count"])
        if tool_call_counts is not None and tool_call_counts.size > 0:
            metrics["tool_call_counts/min"] = float(np.min(tool_call_counts))
            metrics["tool_call_counts/max"] = float(np.max(tool_call_counts))
            metrics["tool_call_counts/mean"] = float(np.mean(tool_call_counts))

    metrics.update(_compute_non_tensor_reward_metrics(batch))
