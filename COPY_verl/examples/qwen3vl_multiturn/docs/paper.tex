...
\title{CreoAgent: A Self-Reflective Multimodal Agent for Autonomous Visual Creation with Dynamic Multi-Tool Orchestration}
\author{%
  Anonymous Author(s) \\
  Affiliation \\
  Address \\
...
\subsection{Multimodal Agents for Visual Creation}
...
\subsection{Tool-Augmented Visual Reasoning}
...
\subsubsection{Structured Reasoning and Tool Invocation}
...
\textbf{Table. Tool set of CreoAgent.}

\begin{table}[t]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Tool} & \textbf{Function} & \textbf{Backend} \\
\midrule
...
\subsubsection{Image Asset Dictionary}
...
\subsubsection{Vision-Grounded Adaptive Reasoning}
...
\subsection{CreoTraj: Multi-Turn Tool-Use Trajectory Dataset}
...
\subsubsection{CreoTraj-SFT (100K Trajectories)}
...
\subsubsection{CreoTraj-RL (10K Instructions)}
...
\subsubsection{Stage 1: Supervised Fine-Tuning (Cold-Start)}
...
\subsubsection{Stage 2: Multi-Tool RL with Trajectory-Level Judge Reward}
...
\subsubsection{Asynchronous Multi-Tool Execution with GPU Isolation}
...
We organize our evaluation into \textbf{main outcome metrics} and \textbf{trajectory behavior metrics}. The main tables only report the former, while the latter are moved to a separate behavior analysis table to keep the core comparison compact and easier to read.

\textbf{Main outcome metrics.}

\begin{itemize}
    \item \textbf{Instruction Following} ($S_{\text{inst}}$): a judge score from 1 to 10 measuring whether the final output faithfully satisfies the user instruction.
    \item \textbf{Visual Quality} ($S_{\text{qual}}$): a judge score from 1 to 10 measuring image fidelity, naturalness, and artifact level.
    \item \textbf{Overall Score} ($S_{\text{overall}}$): the mean of the two final-output scores, i.e., $S_{\text{overall}} = \frac{1}{2}(S_{\text{inst}} + S_{\text{qual}})$.
    \item \textbf{Task Completion Rate} ($\text{CR}$): the percentage of test cases where the system produces a valid final image and ends normally without crashing, looping, or returning an empty result.
    \item \textbf{Tool Accuracy} ($\text{Acc}_{\text{tool}}$): among all generated tool calls, the percentage whose selected tool type is appropriate for the current sub-task, judged by the evaluator model or rule-based matching when the expected tool is unambiguous.
    \item \textbf{Avg. Tool Calls}: the average number of tool invocations per trajectory, used as a lightweight efficiency indicator.
\end{itemize}

\textbf{Trajectory behavior metrics.} These metrics are not included in the main comparison table because they are only defined for tool-agent trajectories and are mainly used to explain *why* RL improves over SFT.

\begin{itemize}
    \item \textbf{Multi-Tool Rate} ($\text{MTR}$): the percentage of trajectories that invoke at least two distinct tools.
    \item \textbf{Invalid Tool Call Rate} ($\text{ITCR}$): the percentage of tool calls that are malformed, contain invalid arguments, or reference a nonexistent image asset id.
    \item \textbf{Premature Termination Rate} ($\text{PTR}$): the percentage of trajectories that emit \texttt{Terminate} before satisfying the major requirements in the instruction.
    \item \textbf{Reasoning Coherence} ($S_{\text{reas}}$): an optional judge score for analyzing whether the trajectory-level reasoning and tool sequence are logically consistent. We keep it for ablations and qualitative analysis, but do not use it as a primary ranking metric in the main result table because it is more subjective than final-output quality and completion rate.
\end{itemize}
...
\begin{table}[t]
\centering
\caption{Main results on CreoTraj-Test. The main table is intentionally compact and focuses on final output quality, task completion, tool selection correctness, and tool-use cost. Tool-specific metrics for QwenImage-Edit are marked as N/A because it does not expose explicit tool trajectories.}
\label{tab:main}
\begin{tabular}{lcccccc}
\toprule
\textbf{Method} & $S_{\text{inst}}$ & $S_{\text{qual}}$ & $S_{\text{overall}}$ & CR & $\text{Acc}_{\text{tool}}$ & Avg. Calls \\
\midrule
...
CreoAgent (full) & \textbf{X.X} & \textbf{X.X} & \textbf{X.X} & \textbf{X.X\%} & \textbf{X.X\%} & X.X \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{RL improves over SFT by learning better long-horizon decisions.} CreoAgent (full) outperforms CreoAgent-SFT by [X.X] points in $S_{\text{overall}}$ and improves CR with a similar or more effective tool-call budget, indicating that RL does not merely polish image quality but also improves trajectory-level decision making.

To make the comparison with QwenImage-Edit more diagnostic, we additionally report results on a \textbf{Complex Multi-Tool Subset} of CreoTraj-Test, where each instruction explicitly requires at least two heterogeneous operations beyond plain editing (e.g., grounding+editing, segmentation+editing+SR, OCR-guided editing, crop+SR).

\begin{table}[t]
\centering
\caption{Results on the Complex Multi-Tool Subset of CreoTraj-Test. This subset is designed to highlight tasks where a single end-to-end editor is structurally insufficient because the instruction requires explicit perception, region selection, or enhancement operations.}
\label{tab:complex_subset}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & $S_{\text{inst}}$ & $S_{\text{overall}}$ & CR \\
\midrule
QwenImage-Edit & X.X & X.X & X.X\% \\
CreoAgent-SFT & X.X & X.X & X.X\% \\
CreoAgent (full) & \textbf{X.X} & \textbf{X.X} & \textbf{X.X\%} \\
\bottomrule
\end{tabular}
\end{table}


\subsubsection{Multi-Tool Behavior Analysis}
...
\begin{table}[t]
\centering
\caption{Behavior analysis on CreoTraj-Test. These metrics are only computed for agent trajectories and therefore are reported separately from the main model comparison.}
\label{tab:behavior_analysis}
\begin{tabular}{lccccc}
\toprule
\textbf{Method} & MTR & ITCR & PTR & Avg. Calls & $S_{\text{reas}}$ \\
\midrule
Qwen3-VL-8B (zero-shot w/ tools) & X.X\% & X.X\% & X.X\% & X.X & X.X \\
CreoAgent-SFT & X.X\% & X.X\% & X.X\% & X.X & X.X \\
CreoAgent (full) & \textbf{X.X\%} & \textbf{X.X\%} & \textbf{X.X\%} & X.X & \textbf{X.X} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Results on [Public Benchmark]}
...
\subsubsection{Effect of Image Asset Dictionary}

\begin{table}[t]
\centering
\caption{Ablation on the Image Asset Dictionary.}
\label{tab:ablation_dict}
\begin{tabular}{lcccc}
\toprule
\textbf{Variant} & $S_{\text{overall}}$ & $\text{Acc}_{\text{tool}}$ & CR & MTR \\
\midrule
CreoAgent (full) & \textbf{X.X} & \textbf{X.X\%} & \textbf{X.X\%} & \textbf{X.X\%} \\
...
\subsubsection{Effect of Trajectory-Level Judge Reward}

\begin{table}[t]
\centering
\caption{Ablation on reward design for RL training.}
\label{tab:ablation_reward}
\begin{tabular}{lcccc}
\toprule
\textbf{Reward Design} & $S_{\text{overall}}$ & $S_{\text{inst}}$ & $S_{\text{qual}}$ & MTR \\
\midrule
Trajectory-level judge (ours) & \textbf{X.X} & \textbf{X.X} & \textbf{X.X} & \textbf{X.X\%} \\
...
\subsubsection{Effect of RL Training Data Scale}

\begin{table}[t]
\centering
\caption{Effect of RL instruction set size.}
\label{tab:ablation_rl_scale}
\begin{tabular}{lccc}
\toprule
\textbf{RL Instructions} & $S_{\text{overall}}$ & MTR & Avg. Calls \\
\midrule
1K & X.X & X.X\% & X.X \\
5K & X.X & X.X\% & X.X \\
10K (full) & \textbf{X.X} & \textbf{X.X\%} & X.X \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{SFT Data Scale Analysis}

\begin{table}[t]
\centering
\caption{Effect of SFT data scale on downstream RL performance.}
\label{tab:ablation_sft_scale}
\begin{tabular}{lccc}
\toprule
\textbf{SFT Trajectories} & $S_{\text{overall}}$ (SFT-only) & $S_{\text{overall}}$ (after RL) & $\Delta$ \\
\midrule
10K & X.X & X.X & +X.X \\
50K & X.X & X.X & +X.X \\
100K (full) & X.X & \textbf{X.X} & +X.X \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Multi-Tool Orchestration Examples}
...
\subsubsection{Adaptive Behavior Analysis}
...
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{SFT Stage} & \textbf{RL Stage} \\
\midrule
