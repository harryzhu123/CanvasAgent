#!/usr/bin/env python3
"""
Outcome-ablation reward for multi-turn visual tool-use RL training.

This reward keeps the process reward, efficiency penalties, and fatal error
gate from ``multiturn_reward.py`` unchanged, while removing the outcome-layer
judges from the final reward:
  - image_prompt_judge
  - aesthetic_judge

For monitoring consistency, those two judges are still computed and returned in
the diagnostics so validation curves remain directly comparable with the
original ``multiturn_reward.py`` setup.

The final reward is still normalized to [0, 1]. Since the outcome layer is
...
_SPEC = importlib.util.spec_from_file_location("multiturn_reward_base_ablation_outcome", _BASE_PATH)
...
WEIGHT_IMAGE_PROMPT_JUDGE = 0.0
WEIGHT_AESTHETIC_JUDGE = 0.0
...
OUTCOME_REWARD_COMPONENT_MAX = 0.0
OUTCOME_REWARD_BUDGET = 0.0
PROCESS_REWARD_BUDGET = max(TOTAL_REWARD_POSITIVE_MAX - OUTCOME_REWARD_BUDGET, 0.0)
FALLBACK_SCORE = _BASE.FALLBACK_SCORE
MONITOR_WEIGHT_IMAGE_PROMPT_JUDGE = _BASE.WEIGHT_IMAGE_PROMPT_JUDGE
MONITOR_WEIGHT_AESTHETIC_JUDGE = _BASE.WEIGHT_AESTHETIC_JUDGE


def compute_score(data_source, solution_str, ground_truth, extra_info=None, **kwargs):
    """Compute ablated reward while preserving outcome-judge diagnostics."""
    ...
    judge_score = (
        MONITOR_WEIGHT_IMAGE_PROMPT_JUDGE * image_prompt_judge_score
        + MONITOR_WEIGHT_AESTHETIC_JUDGE * aesthetic_judge_score
        + WEIGHT_TRAJECTORY_JUDGE * trajectory_judge_score
    ) / max(
        MONITOR_WEIGHT_IMAGE_PROMPT_JUDGE + MONITOR_WEIGHT_AESTHETIC_JUDGE + WEIGHT_TRAJECTORY_JUDGE,
        1e-6,
    )
    outcome_reward = 0.0
    ...
    if PROCESS_REWARD_COMPONENT_MAX > 1e-6:
        process_reward = PROCESS_REWARD_BUDGET * _BASE._clamp01(process_positive / PROCESS_REWARD_COMPONENT_MAX)
    else:
        process_reward = 0.0
