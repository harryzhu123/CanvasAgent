...
class MultiTurnSFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: str | list[str], tokenizer, config=None):
        ...
        self.messages_key = multiturn_config.get("messages_key", "conversation")
        self.images_key = multiturn_config.get("images_key", "image")
        ...
        self.tokenizer: Union[PreTrainedTokenizer, ProcessorMixin] = tokenizer
        
        # 检测是否为processor
        self.is_processor = isinstance(self.tokenizer, ProcessorMixin)
    ...
    def _read_files_and_process(self):
        ...
        self.images = self.dataframe[self.images_key].apply(series_to_item).tolist()
    ...
    def _load_images_from_images_list(self, image_path: str) -> list[Image.Image]:
        """
        从self.images中加载图像
        
        Args:
            image_path: 图像路径(字符串或路径)
        ...
        # 处理image_path可能是numpy array的情况
        if isinstance(image_path, np.ndarray):
            image_path = image_path.item() if image_path.size == 1 else image_path.tolist()
        ...
        if isinstance(image_path, list):
            for path in image_path:
                if path and isinstance(path, str):
                    # 添加路径前缀
                    full_path = self.image_path_prefix + path if self.image_path_prefix else path
                    try:
        ...
        # 如果是单个路径字符串
            ...
            full_path = self.image_path_prefix + image_path if self.image_path_prefix else image_path
            try:
                img = Image.open(full_path).convert("RGB")
                images.append(img)
            except Exception as e:
    ...
    def _process_message_tokens(
        ...
        """
        # 使用实际的tokenizer进行编码
        actual_tokenizer = self.tokenizer.tokenizer if self.is_processor else self.tokenizer
        
        # 构建apply_chat_template的参数
        template_kwargs = {
            "tokenize": False,
        ...
        if is_assistant:
            ...
            generation_prompt_tokens = actual_tokenizer.encode(
                generation_prompt_text,
                add_special_tokens=False,
            )
            _message_tokens = actual_tokenizer.encode(
            ...
            message_tokens = actual_tokenizer.encode(
    ...
    def __getitem__(self, item):
        ...
        if self.is_processor:
            ...
            images = self._load_images_from_images_list(image_path)
            if not images:  # 如果没有图像,设置为None
                images = None

        # First, get the full conversation tokens
        try:
            if self.is_processor:
                # processor需要同时处理文本和图像
                # 先获取文本
                ...
                # 保存其他可能的processor输出(如pixel_values, image_grid_thw等)
                processor_outputs = {
                    k: v[0] if v.dim() > 1 else v 
        ...
        if self.pad_mode == DatasetPadMode.RIGHT:
            if sequence_length < self.max_length:
                # Pad sequences
                pad_token_id = (self.tokenizer.tokenizer.pad_token_id if self.is_processor 
            ...
            # 添加processor的额外输出
            if self.is_processor and processor_outputs:
                result.update(processor_outputs)
...
if __name__ == "__main__":
    """Test script for MultiTurnSFTDataset"""
    import json
    import tempfile
    from transformers import AutoTokenizer
    from verl.utils import hf_processor
    print("=" * 80)
    print("MultiTurnSFTDataset Test")
    print("=" * 80)
    processor = hf_processor('/nfsdata3/xiejianhao/xiejianhao/check/Qwen2.5-VL-7B-Instruct')
    testdataset = MultiTurnSFTDataset(
        parquet_files="/data/zhuhairui/data/merged_all.parquet",
        tokenizer=processor
    )
    print(testdataset[0])
    print("Test passed!")