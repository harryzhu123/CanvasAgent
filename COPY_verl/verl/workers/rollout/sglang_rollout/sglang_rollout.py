...
class SGLangRollout(BaseRollout):
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        # Use per-turn limit if configured in multi-turn mode
        effective_max_new_tokens = self.config.response_length
        if self.config.multi_turn.max_single_turn_length is not None:
            effective_max_new_tokens = self.config.multi_turn.max_single_turn_length
    ...
    async def _handle_engine_generate(
        ...
        # Use per-turn limit if configured in multi-turn mode, otherwise fall back to global response_length
        response_limit = self.config.response_length
        if (
            self.config.multi_turn.enable
            and self.config.multi_turn.max_single_turn_length is not None
        ):
            response_limit = self.config.multi_turn.max_single_turn_length
        max_new_tokens = min(response_limit, self.config.max_model_len - len(generation_prompt_ids) - 1)
